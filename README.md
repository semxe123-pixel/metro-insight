...
단순 시각화가 아니라, 외부 API 데이터를 안정적으로 수집하고 그 상태를 운영자가 확인할 수 있는 대시보드를 목표로 구현했습니다.
...
스케줄러는 `collection_targets`에서 활성화된 대상만 조회해 수집합니다.  
따라서 수집 대상을 바꿀 때 서버 코드를 수정할 필요가 없습니다.
...
이를 통해 데이터 수집이 정상적으로 실행되고 있는지 추적할 수 있도록 했습니다.
...
저장된 데이터뿐 아니라 수집 파이프라인의 상태도 함께 확인하기 위한 기능입니다.
...
오래되었거나 지나치게 먼 도착 예정 정보가 현재 정보처럼 보이지 않도록 했습니다.
...
이를 통해 요청 처리, 비즈니스 로직, DB 접근 책임을 명확히 나누었습니다.
...
결과적으로 수집 대상 변경이 코드 수정 작업에서 관리자 화면 조작으로 바뀌었습니다.

---

### 2. 단순 최신 데이터 표시 → 데이터 신선도 필터 적용

초기에는 MongoDB에 저장된 최신 도착정보를 그대로 보여주었기 때문에, 운행 시간이 아니거나 오래된 데이터가 화면에 남을 수 있었습니다.

이후 다음 조건을 적용해 현재 정보처럼 보일 수 있는 데이터를 걸러냈습니다.
...
수집 성공 여부뿐 아니라 외부 API 호출 상태까지 대시보드에서 확인할 수 있게 되었습니다.
...
이후 `App.jsx`는 데이터 요청과 상태 관리에 집중하고, 화면 구성은 각 컴포넌트가 담당하도록 정리했습니다.
...
배포 환경에서는 localhost를 사용할 수 없기 때문에, Vite 환경변수로 API 주소를 분리했습니다.

```env
VITE_API_BASE_URL=http://localhost:4000
```

배포 환경에서는 같은 변수에 Render 백엔드 URL을 설정했습니다.
```env
...
PowerShell에서 한글 JSON body를 직접 보낼 때 `강남`이 `??`처럼 저장되는 문제가 있었습니다.  
프론트엔드 입력 폼을 통해 수집 대상을 추가하도록 하면서 실제 사용 흐름에서는 문제가 발생하지 않도록 했습니다.

---

### 2. 오래된 지하철 도착정보가 화면에 남는 문제

저장된 최신 데이터를 그대로 보여주면 운행 종료 시간대에도 마지막 도착정보가 남을 수 있었습니다.  
...
프론트엔드에 `localhost`가 직접 들어가 있으면 배포 환경에서 백엔드를 찾을 수 없습니다.  
`VITE_API_BASE_URL` 환경변수로 로컬과 배포 환경의 API 주소를 분리했습니다.
...
이 프로젝트를 하면서 데이터를 가져오는 기능 자체보다, 그 데이터가 안정적으로 들어오고 있는지 확인할 수 있는 구조가 더 중요하다는 점을 체감했습니다.

특히 외부 API는 항상 같은 형태로 응답하지 않을 수 있고, 실시간 데이터는 오래된 값이 화면에 남는 것만으로도 사용자에게 잘못된 정보를 줄 수 있다는 점을 알게 되었습니다.

이번 프로젝트에서는 다음 부분을 중점적으로 경험했습니다.

- 외부 API 연동과 예외 처리
- Node.js 기반 주기적 데이터 수집
- MongoDB 문서 구조 설계
- 수집 로그 기반 운영 모니터링
- React 대시보드 구성과 컴포넌트 분리
- 환경변수 기반 로컬/배포 환경 분리
- Controller-Service-Repository 구조 적용
- Vercel, Render, MongoDB Atlas를 활용한 배포

MetroInsight는 단순히 데이터를 보여주는 화면보다, 수집 작업의 상태까지 함께 확인할 수 있는 구조를 만드는 데 초점을 둔 프로젝트입니다.
